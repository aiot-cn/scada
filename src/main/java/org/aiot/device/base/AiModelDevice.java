...
import org.aiot.device.detector.onnx.*;
...
public class AiModelDevice extends BaseDevice implements Observer,BaseExtend.RMenu{
    ...
    public void loadModel(TAiModel tAiModel){
        ...
        if(tAiModel.getTaskType() == TaskEnum.ocr){
            if(tAiModel.getAlgorithm() == AlgorithmEnum.YOLOv5)
                target = new OnnxYoloOcrDevice();
            if(tAiModel.getAlgorithm() == AlgorithmEnum.DAMO)
                target = new OnnxDamoOcrDevice();
        }else if(tAiModel.getAlgorithm() == AlgorithmEnum.DAMO){
            target = new OnnxDamoYoloDevice();
        }else{
            target = new OnnxDevice();
        }

        if(target != null){
            //target.device = new TDevice(-999L,"AI模型-"+file.getName());
            String classNames = tAiModel.getClassNames();
            OnnxWrapper onnxWrapper = null;
            List<OnnxWrapper> onnxWrappers = target.setModelFile(file);
            if(onnxWrappers != null && onnxWrappers.size() > 0){
                onnxWrapper = onnxWrappers.get(0);
            }
            if(Strings.isNotBlank(tAiModel.getClassNames())){
                String names = tAiModel.getClassNames();
                if(labelMap.containsKey(names))
                    names = labelMap.get(names);
                target.setLabels(names);
            }else if(onnxWrapper != null){
                String[] labels = onnxWrapper.getLabels();
                if(labels.length > 0){
                    String names = String.join(",", labels);
                    tAiModel.setClassNames(names);
                    bs.getDao().updateIgnoreNull(tAiModel);
                }
            }

            aiModelMap.put(tAiModel.getId(),target);
        }
    }

    //卸载模型
    public void unloadModel(TAiModel tAiModel){
        AbstractTarget target = aiModelMap.get(tAiModel.getId());
        if(target != null){
            target.destroy();
            aiModelMap.remove(tAiModel.getId());
            log.info("已卸载模型:"+tAiModel.getModelPath());
        }
    }
    ...
    public List<RMenuOption> menuList() {
        ...
        List<TAiModel> models = bs.getTCache(TAiModel.class);
        for (TAiModel model : models){
            String[] names = model.getModelPath().split("/");
            list.add(new RMenuOption("AI模型/"+names[names.length-1],model.getId()+"",device.getId(),"img"));
        }
    ...
    public Object menuClick(String menu, String val) {
        AbstractTarget at = aiModelMap.get(Long.parseLong(menu));
        if(at != null)
            return at.recognize(val,null);
        return null;
    }
    ...
    public void update(Observable o, Object event) {
        ...
        if(!(arg instanceof TAiModel))
            return;

        TAiModel tAiModel = (TAiModel) arg;
        if(ne.getEventType() == EventEnum.SAVE_BEFORE){
            ...
            TAiModel tAiModel2 = bs.getTCache(TAiModel.class,tAiModel.getId());
            if(tAiModel.getId() == null || tAiModel2 == null || tAiModel2.getMd5() == null ||
                    !Strings.equals(tAiModel2.getModelPath(), tAiModel.getModelPath())
            ){
                double fileSize = file.length() / 1024.0 / 1024.0;
                tAiModel.setFileSize(Math.round(fileSize * 10) / 10.0);
                tAiModel.setMd5(Lang.md5(file));
            }
        }else if(ne.getEventType() == EventEnum.SAVE_AFTER){
            TAiModel aiModelBefore = ne.getData2();
            if(aiModelBefore == null){
                loadModel(tAiModel);
            }else if(tAiModel.getIsRemoved() > 0){
                unloadModel(tAiModel);
            }else if(aiModelBefore.getIsRemoved() > 0 || !Strings.equals(aiModelBefore.getModelPath(), tAiModel.getModelPath())){
                unloadModel(aiModelBefore);
                loadModel(tAiModel);
            }
