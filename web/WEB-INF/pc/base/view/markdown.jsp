...
<div id="markdown"></div>
<textarea id="code" style="display: none">${SRes.getContent()}</textarea>
...
<script>
	...
	var customMenuSave = Cherry.createMenuHook('保存', {
		iconName: 'ok',
		onClick: function (selection,event) {
			// 阻止事件冒泡和默认行为，避免页面滚动
			if (event && event.stopPropagation) {
				event.stopPropagation();
			}
			common.jsonCont("saveRes",{"url":"${SRes.url}","content":cherry.getValue()});
		}
	})
	...
	const basicConfig = {
		...
		callback: {
			...
			urlProcessor(url, srcType) {
				var imagePathPrefix = '${base}';
				if (srcType === 'image' && url && !url.startsWith('http')) {
		...
		editor: {
			...
			defaultModel: 'edit&preview', //默认 还有editOnly、previewOnly
	...
	function myFileUpload(file, callback) {
		...
		//微信默认名称
		if(name == "image.png")
			name = new Date().format("yyyyMMddhhmmss")+".png";
		common.uploadFile([file],{path:"image/md",name:name},function(json){
