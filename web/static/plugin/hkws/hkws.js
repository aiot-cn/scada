...
//WebVideoCtrl.I_SupportNoPlugin();//浏览器是否支持无插件
$(function () {
	// 检查插件是否已经安装过
	...
	WebVideoCtrl.I_InitPlugin("100%", "100%", {
		...
		cbDoubleClickWnd: function (iWndIndex, bFullScreen) {
			//双击默认开启3D
			bFullScreen?clickEnable3DZoom():clickDisable3DZoom();
		},
...
function showOPInfo(szInfo, status, xmlDoc) {
	var szTip = "<div>" + dateFormat(new Date(), "mm:ss") + " " + szInfo;
...
function clickGetAudioInfo() {
	...
	WebVideoCtrl.I_GetAudioInfo(szDeviceIdentify, {
		success: function (xmlDoc) {
			...
			showOPInfo(szDeviceIdentify + " 已获取对讲通道");
		},
		error: function (status, xmlDoc) {
			showOPInfo(szDeviceIdentify + " 未获取对讲通道", status, xmlDoc);
		}
	});
}
...
function clickStartPlayback() {
	var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
		...
		bZeroChannel = false,
		iChannelID = parseInt($("#channels").val(), 10),
		szStartTime = ($("#startTime").val()||"").replace("T"," "),
		szEndTime = ($("#endTime").val() || "").replace("T"," "),
		szInfo = "",
		bChecked = false,//启用转码码流
		iRet = -1;
...
function clickReversePlayback() {
	var t1 = ($("#startTime").val()||"").replace("T"," ");
	var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
		szDeviceIdentify = $("#ip").val(),
		iRtspPort = 554,
		iStreamType = parseInt($("#streamtype").val(), 10),
		bZeroChannel = $("#channels option").eq($("#channels").get(0).selectedIndex).attr("bZero") == "true" ? true : false,
		iChannelID = parseInt($("#channels").val(), 10),
		szStartTime = t1.slice(0,10)+" 00:00:00",
		szEndTime = t1,
		szInfo = "";
